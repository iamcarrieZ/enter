...
function enter_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'enter' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'enter' ),
...
function enter_entry_footer() {
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'enter' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
		echo '</span>';
	}
...
function enter_categorized_blog() {
	...
	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so enter_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so enter_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in enter_categorized_blog.
 */
